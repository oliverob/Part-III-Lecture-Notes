...
$$
 ...
 (T^a_{ij})^* = T_{ji}^a
 $$
 Veltmanns formula (below) means that loops with no reliance on external momentum are always zero under dimensional regularisation (can be seen by adding mass term that you then take to zero):
$$
\int \frac{d^d k}{(2\pi)^d} \frac{1}{k^2} = 0
$$
Remember ghosts are also fermions so give a fermion loop negative sign.\\
Can simplify some expressions by counting dimesnions and remembering we need a positive order of $l$ (including the measure) to have a UV divergence so we can neglect all the terms with smaller orders of $l$\\
When given a term like $A_{\mu}^a A^{b\mu} T_r^a T_r^b = \frac{1}{2} A_{\mu}^a A^{b\mu} \{T_r^a, T_r^b\}$ as the $A$ commute with each other.
       \end{document}
