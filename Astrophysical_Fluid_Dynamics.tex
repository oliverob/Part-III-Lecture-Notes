...
$$
\section{Lecture 24}
$A$ goes from 0 to $\infty$ crossing $A=1$ inbetween as at infinity $\rho = 0$ so $A$ is infinite and at origin $\psi = 0$. We have
$$
R u_{\phi} = \frac{R B_{\phi}}{4 \pi k} + l(\psi), \frac{u_{\phi}}{R} = \frac{k B_{\phi}}{R\rho} + \omega(\psi)
$$
Eliminate $B_{\phi}$ and get:
$$
        (u_{\phi} - \frac{l}{R}) = \frac{\rho}{4 \pi k^2}( u_{\phi} - \omega R) 
$$
$$
u_{\phi} - \frac{ R^2 \omega(\psi) - A^2 l(\psi)}{R(1-A^2)}
$$
Therefore $A=1$ is a special point with $R^2 = \frac{l(\psi)}{\omega(\psi)} \implies R_A = \sqrt{\frac{l(\psi)}{\omega(\psi)}}$ is the Alfven radius/surfce.
For $A<<1$ this is sub-Alfvenic, and we have $u_{\phi} \approx \omega(\psi) R$ - solid body rotation in forced by magnetic stresses! 
$$
A^2 \sim \frac{\epsilon_{kin}}{\epsilon_{mag}}
$$
For $A>>1$ - super Alfvenic and we ahve $u_{\phi} \approx \frac{l(\psi)}{R}$ so now the fluid expands preserving its angular momentum so magnetic stresses are no longer important\\\\
Disk orbits at $\Omega_k = (\frac{GM}{R^3})^{\frac{1}{2}}$ in ideal MHD we have that $\omega(\psi) = \Omega_k(R(\psi))$ as the magnetic field lines are frozzen in the disk. Initial AM of the fluid is $l_0 = \Omega_k (R_0) R_0^2$. At Alfven point $l_A \approx \Omega_k(R_0) R_A^2$. "Lever arm effect" leads to a much larger loss of angular momentum than the fluid would have when it was just part of the disk, the magnetic field is transfering some of the angular momentum into the outflowing wind. Angular momentum flow through the disk is propotiaonl to the angular moemntum flow in the wind $\dot M_d \omega_k(R_o) R_o^2 \sim \dot M_w \omega_k(R_0) R_A^2$ so $\frac{\dot M_d}{\dot M_\omega} = (\frac{R_A}{R_0})^2$. Sometimes called the slingshot effect - this is very important need to look into this. Is a very efficent way to lose angular momentum very a small amount of fluid if the flow is signitficantly mangetised and the alfven radius is large. \\\\
How do you launch the wind from the disk?\\
New integral of motion (Jacobi integral):
$\epsilon'(\psi) = \epsilon(\psi) - \omega(\psi) l(\psi) =  \frac{1}{2}u^2 + \phi + h - \frac{\omega R B_{\phi}}}{4 \pi k} - \omega( Ru_{\phi} - \frac{RB_{\phi}}{4\pi k} = \frac{1}{2} u_p^2 + \frac{1}{2} - \omega Ru_{\phi} + \frac{1}{2}(\omega R)^2 + h + \phi$
$$
