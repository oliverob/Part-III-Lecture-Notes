...
To explain the mapping first we must define a partial order $\prec$ over a set of binary strings. For $\alpha = \alpha_{l-1}\ldots \alpha_0$ and $\beta = \beta_{l-1}\ldots \beta_0$, if $\alpha_i = \beta_i$ for $i\geq i_0$ and $\beta_i = 1$ for $i < i_0$ and $\alpha \neq \beta$, then $\alpha \prec \beta$. 
\begin{itemlist}
\item Every qubit $\ket{q_j}$ stores the parity of a set of $2^x$ modes for some $x \geq 0$
\item For even values of $j$, $\ket{q_j}$ stores the occupation number of the $j$-th mode
\item For odd values of $j$, it stores the parity of a subset of adjacent modes with index less than or equal to $j$:
        \begin{romanlist}
        \item When $\log_2 (j +1)$, the set is all the modes less than or equal to $j$
        \item 
        \end{romanlist}
\end{itemlist}
